...
def _parse_trusted_roots() -> frozenset[str]:
    ...
    - GSMA Governance AID (Sprint 62: vetter constraint trust chain)
...
# =============================================================================
# SPRINT 83: RUNTIME TRUSTED ROOTS STATE
# =============================================================================

import asyncio as _asyncio


# Advisory labels for well-known trusted root AIDs.
# Helps operators understand each root's purpose in the admin UI.
KNOWN_ROOT_LABELS: dict[str, dict] = {
...
# Note: GSMA root AID is deployment-specific (generated by bootstrap-issuer.py).
# The admin UI will show domain="vetter" for any AID not in KNOWN_ROOT_LABELS if
# operators annotate it. Currently GSMA roots are unlabelled (no fixed SAID).
#
# Known limitation (Sprint 83): verify_vetter_constraints() does NOT validate
# VetterCertification issuer chain back to trusted roots. Removing the GSMA root
# from TRUSTED_ROOT_AIDS does NOT disable vetter constraint evaluation. Full vetter
# chain trust validation is deferred to a future sprint.


class _TrustedRootsStore:
    """Asyncio-safe mutable store for trusted root AIDs.

    Mutations are lock-protected. Reads use snapshot() which returns an
    immutable frozenset — callers hold the snapshot for one request; concurrent
    mutations cannot affect it. An empty set is valid (fail-closed emergency mode):
    the existing chain validation code fails for any credential when no issuer
    matches a trusted root.
    """
    ...
    async def snapshot(self) -> frozenset[str]:
        """Atomically capture current roots as an immutable frozenset."""
    ...
    async def remove(self, aid: str) -> frozenset[str]:
        """Remove an AID. Raises KeyError if not present. Empty set is allowed."""
        async with self._lock:
    ...
    def current_sync(self) -> frozenset[str]:
        """Synchronous read without lock — for non-security UI/display paths only."""
        return frozenset(self._roots)
...
async def get_trusted_roots_snapshot() -> frozenset[str]:
    """Atomically snapshot the current trusted roots for one request.

    Must be called once per verification request at the boundary in verify.py;
    the returned frozenset is carried through all verification phases and cache
    operations. Concurrent admin mutations cannot affect an in-flight snapshot.
    """
    return await _trusted_roots_store.snapshot()


def get_trusted_roots_current() -> frozenset[str]:
    """Synchronous read of current trusted roots — for UI/display paths only.

    Do NOT use in verification pipeline — use get_trusted_roots_snapshot() there.
    This is eventually-consistent (no lock) and suitable only for non-security
    display operations (e.g., credential graph visualization in the explorer UI).
    """
    return _trusted_roots_store.current_sync()


# Admin token for mutation endpoints (Sprint 83).
# If set, all /admin/* mutation endpoints require Authorization: Bearer <token>.
# If set, read-only endpoints also require the token.
# If not set, mutation endpoints return 503 (fail-closed by default).
