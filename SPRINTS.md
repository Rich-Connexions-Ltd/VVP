...
- **Backward compatibility (CRITICAL)**: The verifier MUST continue to support existing (non-multichannel-aware) dossiers that use base schemas (LE `ENPXp1vQ...`, Brand `EBIFDhtS...`, TNAlloc `EFvnoHDY7I-...`) without `certification` edges. The verifier distinguishes between legacy and multichannel-aware dossiers by inspecting the credential schema SAIDs: base schemas have no `certification` edge definition, extended schemas (`EPknTwPp...`, `EK7kPhs5...`, `EGUh_fVL...`) do. Legacy dossiers skip vetter constraint checks entirely and return `vetter_constraints = None` (not `INDETERMINATE`). This is a non-negotiable constraint — multichannel features must be purely additive with zero regression to existing call flows.
...
- **Verifier backward compatibility**: Existing dossiers using base schemas (without `certification` edges) continue to verify successfully with no vetter constraint warnings or errors. The verifier distinguishes legacy vs. multichannel dossiers by schema SAID and only applies constraint checks to extended-schema credentials.
- All existing tests continue to pass (no regressions)
- New tests cover CRUD, association lifecycle, constraint visibility, and edge injection
- New regression tests confirm existing E2E call flow is unaffected by multichannel changes
...
4. **Multiple enforcement points**: Enforcement at verification **MUST** occur. But enforcement SHOULD also happen at:
   ...
   This sprint implements all three issuer-side enforcement points plus the end-to-end SIP/WebRTC propagation.
...
| Schema registry: all extended SAIDs registered | 40 | COMPLETE | `KNOWN_SCHEMA_SAIDS` |
...
1. No GSMA root of trust — no trust chain from governance body → VetterCertification
2. No issuance-time validation — credentials issued with any scope regardless of vetter authority
3. No dossier creation-time validation — dossier builder doesn't check constraints
4. No signing-time validation — `/vvp/create` doesn't check constraints before signing
5. No `X-VVP-Vetter-Status` SIP header — verifier returns `vetter_constraints` in JSON but it's never propagated to SIP
6. No WebRTC display of vetter violations — `vvp-display.js` has no UI for vetter constraint warnings
...
#### Phase 3: Issuance-Time Constraint Enforcement
...
#### Phase 4: Dossier & Signing-Time Enforcement
...
#### Phase 5: SIP Header Propagation
...
- [ ] **Add vetter constraint display to `vvp-display.js`** — Extend the VVP display panel to show vetter status:
  - Below the existing status badge, add a **vetter constraint badge**:
    ...
    |--------|-------|-----------|------|
    | `PASS` | "Vetter Verified" | `vvp-vetter-pass` | check |
    | `FAIL-ECC` | "Mis-vetted TN" | `vvp-vetter-fail` | warning |
    | `FAIL-JURISDICTION` | "Unauthorized Jurisdiction" | `vvp-vetter-fail` | warning |
    | `FAIL-ECC-JURISDICTION` | "Mis-vetted TN & Jurisdiction" | `vvp-vetter-fail` | warning |
...
- [ ] **Vetter constraint detail panel** — When the vetter status badge is clicked, show an expandable detail section with constraint failure details
...
- [ ] **E2E test: Constraint pass** — Run `system-health-check.sh --e2e` and verify all constraint checks pass end-to-end
...
- **Trust chain depth**: The full chain is: GSMA root AID (in `TRUSTED_ROOT_AIDS`) → GSMA Governance Credential → VetterCertification → Extended TN/LE/Brand → Dossier. The verifier's existing chain walker already handles arbitrary depth.
...
- **Backward compatibility (CRITICAL)**: The verifier MUST continue to support existing (non-multichannel-aware) dossiers that use base schemas (LE `ENPXp1vQ...`, Brand `EBIFDhtS...`, TNAlloc `EFvnoHDY7I-...`) without `certification` edges. The verifier distinguishes between legacy and multichannel-aware dossiers by inspecting the credential schema SAIDs: base schemas have no `certification` edge definition, extended schemas (`EPknTwPp...`, `EK7kPhs5...`, `EGUh_fVL...`) do. Legacy dossiers skip vetter constraint checks entirely and return `vetter_constraints = None` (no `X-VVP-Vetter-Status` header, no badge). Extended-schema dossiers get the full constraint evaluation pipeline. This is a non-negotiable constraint — multichannel features must be purely additive with zero regression to existing call flows.
...
- Sprint 44 (SIP Redirect Verification Service) — SIP verify handler and X-VVP header framework — COMPLETE
...
- **Verifier backward compatibility**: Existing dossiers using base schemas (without `certification` edges) continue to verify successfully — no vetter constraint headers, no vetter badges, no warnings. The verifier identifies legacy vs. multichannel dossiers by schema SAID and applies vetter constraint checks only to extended-schema credentials.
- Bootstrap provisions a VetterCertification from GSMA and uses extended schemas
- E2E test passes with full constraint chain: issuance → dossier → signing → verification → SIP → WebRTC
- E2E regression test confirms existing base-schema dossier flow is unaffected
- All existing tests continue to pass (no regressions)
- New tests cover all constraint enforcement paths, GSMA root, SIP header mapping, WebRTC display, and legacy dossier backward compatibility
...
The knowledge base serves three audiences:
1. **Claude Code** (Tiers 1-3) — CLAUDE.md files and `knowledge/` docs are loaded as context during pair programming. Stale docs cause Claude to make incorrect assumptions about APIs, models, and architecture.
2. **Codex Reviewer** — The context pack (`codex/skills/keri-acdc-vlei-vvp/references/`) is assembled from reference docs. Outdated references lead to incorrect review findings.
3. **Human developers** — API reference, data models, and architecture docs are the primary onboarding material.

### Scope: Files to Audit

| File | Tier | Last Substantive Update | Key Gaps |
|------|------|------------------------|----------|
| `CLAUDE.md` | 1 | Sprint 64 | Missing Sprint 65 credential UI, dossier readiness endpoint |
| `MEMORY.md` | 1 | Sprint 65 | Reasonably current — verify accuracy |
| `services/issuer/CLAUDE.md` | 2 | ~Sprint 53 | Missing dossier wizard, readiness API, schema-driven credential UI, organization names endpoint, managed credentials model |
| `services/verifier/CLAUDE.md` | 2 | ~Sprint 40 | Missing vetter constraint models, extended schema handling |
| `common/CLAUDE.md` | 2 | ~Sprint 40 | Missing schema registry updates, new ACDC models |
| `knowledge/api-reference.md` | 3 | Sprint 65 (partial) | Missing many issuer endpoints added in Sprints 58-65 (dossier CRUD, credential issuance, schema browser, org management) |
| `knowledge/data-models.md` | 3 | Sprint 65 (partial) | Missing DossierCreateRequest, credential models, organization models, API key models |
| `knowledge/architecture.md` | 3 | ~Sprint 40 | Missing issuer multi-tenancy, SSO auth, dossier assembly pipeline, SIP infrastructure |
| `knowledge/schemas.md` | 3 | ~Sprint 40 | Missing dossier schema, extended schemas, vetter certification schema |
| `knowledge/deployment.md` | 3 | ~Sprint 53 | Missing LMDB lock handling, OIDC migration, new repo URL |
| `knowledge/test-patterns.md` | 3 | ~Sprint 40 | Missing issuer test patterns, async test fixtures, mock credential helpers |
...
These are the most impactful — loaded automatically when working in a service directory.

- [ ] **`services/issuer/CLAUDE.md`** — Full refresh:
  - Document all current API routers and their endpoints (health, identity, credential, dossier, organization, vvp, schema, registry)
  - Document the authentication model (SSO via Azure AD, API keys, Principal model, role-based access)
  - Document the dossier creation wizard flow (4-step UI, POST /dossier/create)
  - Document the dossier readiness endpoint (GET /dossier/readiness)
  - Document schema-driven credential creation (edge block parsing, schema browser)
  - Document the KERI integration layer (Habery, witness management, credential issuance)
  - Document the database models (Organization, User, UserOrgRole, OrgAPIKey, ManagedCredential, Dossier)
  - Document key configuration settings and environment variables
  - Document the static file serving pattern (`/static/` mount, `/ui/` routes)

- [ ] **`services/verifier/CLAUDE.md`** — Refresh:
  - Verify the 11-phase verification pipeline description is accurate
  - Add vetter constraint checking (Sprint 40) if not already documented
  - Document the dossier public access endpoint
  - Document any API changes since Sprint 40

- [ ] **`common/CLAUDE.md`** — Refresh:
  - Document current schema registry contents (all SAIDs, names, types)
  - Document ACDC models (credential data classes)
  - Document dossier models
  - Document canonical serialization utilities

#### Phase 2: Tier 3 Knowledge Files

- [ ] **`knowledge/api-reference.md`** — Comprehensive endpoint audit:
  - Walk every FastAPI router in both services and document every endpoint
  - Include request/response models, auth requirements, query parameters
  - Group by service and functional area
  - Flag any undocumented endpoints

- [ ] **`knowledge/data-models.md`** — Comprehensive model audit:
  - Walk every SQLAlchemy model in `services/issuer/app/db/models.py`
  - Walk every Pydantic model in `services/issuer/app/api/models.py` and `services/verifier/app/vvp/api_models.py`
  - Walk every data class in `common/`
  - Include field types, descriptions, relationships

- [ ] **`knowledge/architecture.md`** — Major update:
  - Add the issuer service architecture (added Sprint 28+)
  - Document the multi-tenancy model
  - Document the SSO authentication flow
  - Document the dossier assembly pipeline
  - Document the SIP infrastructure (signing, verification, brand display)
  - Update the system diagram

- [ ] **`knowledge/schemas.md`** — Schema registry refresh:
  - List every schema JSON in `services/issuer/app/schema/schemas/`
  - Document each schema's SAID, purpose, edge structure, and attribute fields
  - Include the dossier schema and its edge block definition
  - Document extended vs. base schema distinction

- [ ] **`knowledge/deployment.md`** — Deployment refresh:
  - Update CI/CD pipeline description (new repo URL, OIDC)
  - Document LMDB lock handling during deployment
  - Document the 4-phase stop procedure
  - Document PBX deployment via Azure CLI
  - Update Azure Container App configuration

- [ ] **`knowledge/test-patterns.md`** — Test pattern refresh:
  - Document issuer test patterns (async fixtures, mock credential helpers, DB setup)
  - Document the `_full_cred_set()` helper pattern
  - Document PostgreSQL VARCHAR(44) constraint for test SAIDs
  - Document the `run-tests.sh` wrapper and libsodium setup

- [ ] **`knowledge/verification-pipeline.md`** — Verify and update:
  - Confirm the 11-phase description matches current code
  - Add vetter constraint checking if missing
  - Document dossier public access verification

- [ ] **`knowledge/keri-primer.md`** — Verify accuracy (likely minimal changes)
...
- [ ] **`CLAUDE.md`** — Incremental update:
  ...
  - Add any new key files or directories
  - Verify all script references are accurate
  - Update service URLs table if needed

- [ ] **`MEMORY.md`** — Verify accuracy of all sections
...
- [ ] **`codex/skills/keri-acdc-vlei-vvp/references/source-map.md`** — Update:
...
- [ ] **Cross-reference check** — Verify that:
  ...
  - Every DB model in code appears in `knowledge/data-models.md`
  - Every schema JSON has an entry in `knowledge/schemas.md`
  - CLAUDE.md project structure matches `ls -R` output
  - All environment variables referenced in code are documented in `knowledge/deployment.md`
...
- **Context pack size budget** — The reviewer context pack has a ~500-line budget. Keep reference docs concise — summaries and tables, not full API docs.
...
- All Tier 2 CLAUDE.md files accurately describe their service's current API, architecture, and patterns
- `knowledge/api-reference.md` documents every endpoint in both services
- `knowledge/data-models.md` documents every model (SQLAlchemy, Pydantic, dataclass)
...
## Sprint 68b: Issuer KERI Agent Client Migration (COMPLETE)

**Goal:** Migrate all issuer routers and the DossierBuilder from direct `app.keri.*` imports to `KeriAgentClient` HTTP client calls, completing the issuer's decoupling from KERI/LMDB.

**Deliverables:**
- [x] Migrate all 15 API routers to use `get_keri_client()` instead of `get_credential_issuer()`, `get_identity_manager()`, `get_registry_manager()`, `get_witness_publisher()`
- [x] Migrate `DossierBuilder` to use `KeriAgentClient` for credential lookups, CESR retrieval, and edge resolution
- [x] Create stateful `MockKeriAgentClient` in conftest.py with identity/registry/credential tracking
- [x] Fix all test patch targets for migrated modules
- [x] 787 issuer tests pass (+ 1844 verifier), 0 failures, 12 skipped

**Scope**: Router-level import migration. Replaces all `app.keri.*` imports in API routers and DossierBuilder with `get_keri_client()` calls. Local DossierBuilder and PASSporT signing are retained (migrated to use keri_client for KERI operations). Full agent-side dossier building and VVP attestation signing are deferred to a future sprint.

**Key Technical Decisions:**
- Witness publishing is delegated to the KERI Agent (routers no longer manage per-witness results)
- `_credential_to_json()` reconstructs ACDC-like JSON from `CredentialResponse` DTO fields
- Per-credential TEL retrieval deferred (agent doesn't expose per-credential TEL endpoint yet)
- Vetter `service.py` / `constraints.py` deferred — they use `reger.creds.get()` directly for deep KERI store access, but all 76 vetter tests pass
- Full agent-side dossier building and VVP attestation signing deferred to future sprint

**Files Modified:**
| File | Change |
|------|--------|
| `app/api/identity.py` | `get_identity_manager()` → `get_keri_client()` |
| `app/api/registry.py` | `get_registry_manager()` → `get_keri_client()` |
| `app/api/credential.py` | `get_credential_issuer()` → `get_keri_client()` |
| `app/api/dossier.py` | `get_credential_issuer()` → `get_keri_client()` |
| `app/api/vvp.py` | `get_credential_issuer()` → `get_keri_client()` |
| `app/api/organization.py` | Mixed — KERI calls via client |
| `app/api/health.py` | `get_identity_manager()` → `get_keri_client()` |
| `app/api/admin.py` | `get_credential_issuer()` → `get_keri_client()` |
| `app/api/tn.py` | `get_credential_issuer()` → `get_keri_client()` |
| `app/api/vetter_certification.py` | `get_credential_issuer()` → `get_keri_client()` |
| `app/dossier/builder.py` | Full migration to `get_keri_client()` |
| `tests/conftest.py` | MockKeriAgentClient with stateful tracking |
| `tests/test_identity.py` | Skip 2 OOBI base URL tests |
| `tests/test_dossier_readiness.py` | Patch target migration (10 tests) |
| `tests/test_org_switching.py` | Issuer-binding mock rewrite |

**Exit Criteria:**
- [x] All 15 routers call `get_keri_client()` instead of direct KERI managers
- [x] DossierBuilder uses `get_keri_client()` for all credential operations
- [x] No router or dossier builder imports from `app.keri.identity`, `app.keri.registry`, `app.keri.issuer`, or `app.keri.witness`
- [x] Full test suite passes (787 issuer + 1844 verifier, 12 skipped)
...
After Sprint 68, the KERI Agent owns all LMDB/keripy state and the Azure Files mount. But the fundamental deployment problem remains: LMDB relies on POSIX file locks for writer exclusion, and Azure Files (SMB) does not correctly implement them. Every KERI Agent deploy risks corruption (`MDB_BAD_DBI`, `MDB_BAD_TXN`) when the outgoing and incoming containers both attempt to open the same LMDB files. The fragile 3-phase stop sequence (deactivate → poll → 10s buffer) narrows the race but cannot eliminate it.

This sprint solves the root cause by making LMDB ephemeral — each container starts with a fresh local LMDB and deterministically rebuilds all KERI state (identities, registries, credentials) from key seeds and configuration stored in PostgreSQL. Since each container uses its own local filesystem, POSIX locks work correctly, there is no shared-state contention, and deploys become safe and fast.

### Current State (after Sprint 68)

```
KERI Agent Container
├── Azure Files mount (/data/vvp-issuer)    ← SMB, POSIX lock issues
...
├── Startup: open existing LMDB → load identities (~3 min on Azure Files)
├── Deploy: 3-phase stop → deploy → verify (~5-7 min, risk of corruption)
└── Scaling: max_replicas=1 (LMDB single-writer constraint)
```

### Target State (after Sprint 69)

```
KERI Agent Container
├── Local ephemeral storage (/tmp/vvp-issuer)  ← proper POSIX locks
│   ├── keri/db/vvp-issuer/data.mdb            ← rebuilt on startup
│   ├── keri/ks/vvp-issuer/data.mdb            ← rebuilt from seeds
│   └── keri/reg/vvp-issuer/data.mdb           ← rebuilt from seeds
├── PostgreSQL (shared)
│   ├── keri_seeds table                        ← key seeds + salts per identity
│   └── keri_credentials table                  ← credential metadata for rebuild
├── Startup: create fresh LMDB → rebuild from seeds (~10-15s on local disk)
├── Deploy: standard rolling update (~30-60s, zero corruption risk)
└── Scaling: still max_replicas=1 (KERI Agent is stateful per-request, but
│            could potentially scale to read replicas in future)
```

### Deliverables

#### Phase 1: Key Seed Persistence
...
- [x] **`keri_seeds` database table** — New SQLAlchemy model:
  ...
  salt:           str — hex-encoded salt passed to Habery
  seed:           str — encrypted seed material (optional, for non-salt-derived keys)
  algo:           str — key algorithm ("salty" for salt-derived, "randy" for random)
  icount:         int — initial key count
  ncount:         int — next key count
  isith:          str — signing threshold
  nsith:          str — next signing threshold
  witness_aids:   JSON — list of witness AIDs at inception
  expected_aid:   str — the AID this seed should produce (for verification)
  created_at:     datetime
  updated_at:     datetime
...
- [x] **Startup sequence** — KERI Agent lifespan:
  ...
  4. Run `MockVLEIManager.initialize()` — rebuild mock vLEI trust chain
...
- [ ] **Remove Azure Files infrastructure** — Once ephemeral LMDB is proven stable:
  ...
  - Delete `vvpissuerdata` Azure Storage account (or repurpose for other uses)
  - Remove Azure Files references from CLAUDE.md, knowledge files, MEMORY.md
...
- [x] **Knowledge updates** — Update `knowledge/deployment.md`, `knowledge/architecture.md`, `CLAUDE.md` to reflect:
  ...
  - Seed-based deterministic rebuild model

### Technical Notes

- **Salt determinism** — keripy's `Habery(salt=...)` uses the salt to derive all signing keys deterministically. Given the same salt + algorithm + key counts + thresholds, `makeHab()` produces the same AID. This is the foundation of the rebuild strategy.

- **Credential SAID stability** — Credential SAIDs are computed from the credential content (attributes, edges, schema, issuer AID). If the issuer AID is the same (from deterministic salt), and the attributes/edges are the same, the credential SAID will be the same. This ensures dossier references remain valid across restarts.

- **Registry nonce** — Registry creation in keripy uses a nonce. To get deterministic registry keys, the nonce must also be persisted and replayed during rebuild.

- **Edge references** — Credentials with edges (e.g., Brand credential → LE credential → QVI credential → GLEIF root credential) must be rebuilt in dependency order. The `KeriStateBuilder` sorts credentials by their edge graph (topological sort) before re-issuing.

- **Witness publishing** — On rebuild, the KERI Agent re-publishes KEL events to witnesses. This is idempotent — witnesses already have the events from previous runs and will accept or ignore duplicates.

- **PostgreSQL dependency** — The KERI Agent now requires PostgreSQL connectivity at startup (to read seeds). This is acceptable since PostgreSQL is already a hard dependency for the Issuer API. The KERI Agent's PostgreSQL access is read-only except during identity/credential creation.

- **Local LMDB sizing** — LMDB on local ephemeral storage is bounded by the container's available disk. Azure Container Apps containers have ~2GB ephemeral storage. Current LMDB usage is ~50MB, so this is ample.

- **Seed encryption** — Seeds stored in PostgreSQL should be encrypted at rest. Options: (a) PostgreSQL column-level encryption via pgcrypto, (b) application-level encryption with a key from Azure Key Vault, (c) rely on Azure Database for PostgreSQL's transparent data encryption (TDE). Option (c) is simplest since TDE is enabled by default on Azure.

### Risks and Mitigations

| Risk | Likelihood | Impact | Mitigation |
|------|------------|--------|------------|
| Seed mismatch → wrong AID after rebuild | Low | High | Verify `expected_aid` against generated AID on every rebuild. Alert if mismatch. |
| Credential SAID drift (different SAID after rebuild) | Medium | High | Persist credential nonces and attributes. Verify rebuilt SAIDs match stored SAIDs. |
| Startup time regression (rebuild too slow) | Low | Low | Local LMDB I/O is ~100x faster than Azure Files. Target <15s for full rebuild. |
| PostgreSQL unavailable during KERI Agent startup | Low | High | Retry with backoff (up to 60s). KERI Agent cannot start without seeds. |
| Ephemeral storage full | Very Low | Medium | LMDB uses ~50MB; containers have ~2GB. Monitor and alert. |
| Seed loss (PostgreSQL data loss) | Very Low | Critical | Regular PostgreSQL backups (Azure managed). Seed export endpoint for manual backup. |

### Dependencies

- Sprint 68 (KERI Agent Service Extraction — provides the standalone KERI Agent with its own deploy lifecycle)

### Exit Criteria

- KERI Agent uses local ephemeral storage for LMDB (no Azure Files mount)
- All KERI identities are rebuilt deterministically from PostgreSQL-persisted seeds on every startup
- AIDs are stable across container restarts (same salt → same AID)
- Credential SAIDs are stable across container restarts
- CI/CD deploy for KERI Agent uses standard rolling update (no 3-phase stop)
- Deploy completes in <2 minutes with zero risk of LMDB corruption
- `bootstrap-issuer.py` works without LMDB wipe step
- Startup rebuild completes in <15 seconds
- Seed export endpoint available for disaster recovery
- All existing tests pass (no regressions)
- E2E SIP loopback call succeeds after a KERI Agent restart
...
Sprint 70 closes this gap by adding automatic witness re-publishing to the StateBuilder's rebuild lifecycle.

### Deliverables

- [x] **Phase 6 in StateBuilder** — `_publish_to_witnesses()` with concurrent `asyncio.gather`, seed-based identity filtering
- [x] **RebuildReport extension** — `witnesses_published` count and `witness_publish_seconds` timing
- [x] **Phase 2 receipt distribution** — keripy-proven rct event approach with -B WitnessIdxSigs for `fullyWitnessed()`
- [x] **Inception cache** — Uses `hab.iserder.saidb` to bypass LMDB sn=0 corruption after registry/credential anchoring
- [x] **Graceful degradation** — Failures logged as warnings, startup continues regardless
- [x] **Tests** — 8 new tests (133 total): witness receipt distribution, HTTP simulation, report formatting, publish success/failure
- [x] **Knowledge updates** — architecture.md, deployment.md updated

### Technical Notes

- Two-phase witness publishing: Phase 1 sends inception to `/receipts`, Phase 2 distributes receipt event with all witness indexed sigs to root `/` endpoint
- Phase 2 uses the keripy-proven `eventing.receipt()` + `eventing.messagize(serder, wigers)` approach from `test_witness.py`
- `/publish` endpoint uses `get_inception_msg()` (not `get_kel_bytes()`) to avoid LMDB sn=0 corruption
- Inception cache in StateBuilder captured right after `makeHab()`, before registry/credential ops modify LMDB
- Publishing is idempotent — witnesses accept or ignore duplicate events
- Only publishes identities from seed store (skip keripy internals like Habery signator)
- Verified: 69/69 identities published in 2.1s, all 3 witnesses return OOBI 200
...
- [x] After full redeploy, all identities discoverable via witness OOBI resolution without manual intervention
- [x] StateBuilder logs show witness publishing phase with timing (69/69, 2.1s)
- [x] No startup failures if witnesses are temporarily unreachable
