...
from common.vvp.dossier.config import TEL_CLIENT_TIMEOUT_SECONDS
...
class TELClient:
    ...
    async def check_revocation(
        ...
        """
        log.info(
            f"check_revocation: cred={credential_said[:20]}... "
        ...
        # No TEL data found
        log.info("  no_tel_data_found: returning UNKNOWN")
    ...
    async def _query_via_oobi(
        self, credential_said: str, registry_said: Optional[str], oobi_url: str
    ) -> RevocationResult:
            ...
            async with httpx.AsyncClient(timeout=self.timeout, verify=False) as client:
                for endpoint in endpoints:
                    ...
                    try:
                        ...
                        log.info(
                            f"    oobi_response: status={resp.status_code} len={len(resp.text)}"
                        )
    ...
    async def _query_witness(
        self, credential_said: str, registry_said: Optional[str], witness_url: str
    ) -> RevocationResult:
        ...
        try:
            async with httpx.AsyncClient(timeout=self.timeout, verify=False) as client:
                ...
                try:
                    ...
                    log.info(
                        f"    witness_response: status={resp.status_code} len={len(resp.text)}"
                    )
    ...
    async def check_revocation_with_fallback(
        ...
        if dossier_data:
            ...
            if dossier_result.status == CredentialStatus.REVOKED:
                log.info("  dossier_shows_revoked: returning immediately")
                return dossier_result
            log.info(
                f"  dossier_status={dossier_result.status.value}: querying witness for live status"
            )
...
def reset_tel_client() -> None:
    """Reset the TEL client singleton (for testing)."""
    global _tel_client
    _tel_client = None
