"""Hierarchical phase timing for pipeline instrumentation.

Collects per-phase wall-clock timing for both issuer and verifier pipelines.
Supports nested sub-phases via dotted names and both sync and async contexts.
...
Sprint 76: Moved from verifier-only to common for shared use.
"""
...
@dataclass
class PhaseTimer:
    """Collects per-phase timing for a single request.
    ...
    def phase(self, name: str) -> Generator[None, None, None]:
        """Context manager for timing a synchronous phase.
    ...
    @asynccontextmanager
    async def aphase(self, name: str) -> AsyncGenerator[None, None]:
        """Async context manager for timing an async phase.

        Usage:
            async with timer.aphase("identity_resolve"):
                identity = await client.get_identity(name)
        """
        self.start(name)
        try:
            yield
