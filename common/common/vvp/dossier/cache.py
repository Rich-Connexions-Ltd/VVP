...
if TYPE_CHECKING:
    from common.vvp.keri.tel_client import ChainExtractionResult, ChainRevocationResult
...
class DossierCache:
    ...
    def __init__(
        ...
        Args:
            ...
            tel_client_factory: Callable that returns a TELClient instance.
                If None, imports get_tel_client from common.vvp.keri.tel_client.
                Services should inject their own factory to use service-specific
                witness pool singletons.
        """
        self._tel_client_factory = tel_client_factory
    ...
    async def _do_revocation_check(
        ...
        try:
            if self._tel_client_factory is not None:
                tel_client = self._tel_client_factory()
            else:
                from common.vvp.keri.tel_client import get_tel_client
...
def get_dossier_cache() -> DossierCache:
    ...
    Configuration is read from common.vvp.dossier.config on first access.
    """
    global _dossier_cache
    if _dossier_cache is None:
        from .config import DOSSIER_CACHE_MAX_ENTRIES, DOSSIER_CACHE_TTL_SECONDS
