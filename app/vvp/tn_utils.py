...
Per VVP §6.3.6, TNAlloc credentials must specify phone number allocations
and child allocations must be subsets of parent allocations.
...
def validate_e164(tn: str) -> None:
    ...
    Per VVP §4.2 and RFC8225, phone numbers must be in E.164 format.
...
def is_subset(child_ranges: List[TNRange], parent_ranges: List[TNRange]) -> bool:
    ...
    Per VVP §6.3.6, a TNAlloc credential's phone number allocation
    must be a subset of its parent TNAlloc's allocation.
